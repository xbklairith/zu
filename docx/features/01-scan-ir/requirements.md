...
| REQ-047 | The system SHALL record on every `type` and `function` node a `shape`, written directly after `hash`: the same SHA-256 as `hash` (REQ-022, merged as in REQ-026) computed after replacing every identifier spelled like the declared name or, for a method, its receiver's base type name with `_`, so a rename or receiver-type rename keeps it. *(Added by feature 02, decision 0003 amended.)* | 0003, feature 02 |
...
| REQ-024 | The system SHALL NOT follow a symbolic link to a directory, SHALL NOT read any file whose resolved path lies outside the scanned root, and SHALL NOT follow a symbolic link whose target (at any hop) is an absolute path, since where it points depends on the checkout location. *(Absolute clause added by feature 02.)* | T6, Privacy and security |
